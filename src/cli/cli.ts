...
import { HEADERS_WITH_CURL, HEADERS_WITH_SAFARI, HEADERS_WITH_WHATSAPP } from '../helpers/RSSFeedHelpers'
...
const definitions =
    addOption('-q, --quiet', 'quiet mode', () => setOutput(() => { }))
        ...
        addCommand('metadata-only <url>', 'Fetch metadata of url', async (url: string) => {
            const data = await fetchHtmlMetaDataOnly(url, { headers: { ...HEADERS_WITH_CURL } })
            output(data)
        })
        .
